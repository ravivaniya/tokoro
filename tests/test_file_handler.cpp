...
TEST_CASE("FileHandler: Path Traversal Prevention", "[file_handler]") {
    ...
    auto res1 = FileHandler::handle_request(req, docroot);
    REQUIRE(res1.status_code == 403);
    
    // URL-encoded traversal
    req.uri = "/%2e%2e/%2e%2e/CMakeLists.txt";
    auto res2 = FileHandler::handle_request(req, docroot);
    REQUIRE(res2.status_code == 403);
    
    // NUL byte injection
    req.uri = "/index.html%00";
    auto res3 = FileHandler::handle_request(req, docroot);
    REQUIRE(res3.status_code == 400);

    // Absolute path
    req.uri = "//etc/passwd";
    auto res4 = FileHandler::handle_request(req, docroot);
